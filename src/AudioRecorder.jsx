...
const queryPinecone = async ({ embedding }) => {
    const response = await fetch(`https://ai-poc-bg9a58h.svc.aped-4627-b74a.pinecone.io/query`, {
        method: "POST",
        headers: {
...
export const AudioRecorder = () => {
    ...
    const FUNCTION_TOOLS = {
        getPreviousConversation: async ({ user_input }) => {
            ...
            const queryResponse = await queryPinecone({ embedding: embedding.data[0].embedding });
    ...
    const upsertEmbeddings = async (data, embeddings) => {
        ...
        await fetch(`https://ai-poc-bg9a58h.svc.aped-4627-b74a.pinecone.io/vectors/upsert`, {
            method: "POST",
            headers: {
                "Api-Key": PINECONE_API_KEY,
                "Content-Type": "application/json",
    ...
    const createEmbeddings = async (text) => {
        ...
        const embedding = await openaiClient.embeddings.create({
            ...
            input: JSON.stringify(
                { full_conversation: text, summary: completion.choices[0].message.content }
            ),
            encoding_format: "float",
        });
